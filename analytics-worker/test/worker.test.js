...
test("DJ mail requires a live signed DJ invite and returns privacy-safe fields", async () => {
  const token = await downloadContext();
  const env = {
    ...baseEnv,
    DB: {
      prepare: (sql) => ({
        bind: () => sql.includes("SELECT id FROM access_invites")
          ? { first: async () => ({ id: "invite" }) }
          : { all: async () => ({ results: [{
            id: "mail-1", sender_name: "Ghosts In Shells",
            reply_to: "d.wright@ghostsinshells.com", subject: "Update",
            body_text: "Private message", received_at: "2026-08-02T00:00:00Z",
            permanent: 1
          }] }) }
      })
    }
  };
  const response = await worker.fetch(request("/v1/dj-mail", {
    method: "POST",
    headers: { Origin: origin, "Content-Type": "application/json" },
    body: JSON.stringify({ invite_context_token: token })
  }), env);
  assert.equal(response.status, 200);
  const result = await response.json();
  assert.equal(result.messages[0].replyTo, "d.wright@ghostsinshells.com");
  assert.equal("sender_address" in result.messages[0], false);
  assert.equal((await worker.fetch(request("/v1/dj-mail", {
    method: "POST",
    headers: { Origin: origin, "Content-Type": "application/json" },
    body: JSON.stringify({ invite_context_token: "bad" })
  }), env)).status, 401);
});

test("inbound DJ mail accepts only the owner envelope sender", async () => {
  let inserted = 0;
  const env = {
    ...baseEnv,
    DB: { prepare: () => ({ bind: () => ({ run: async () => { inserted += 1; } }) }) }
  };
  const raw = "Message-ID: <one@example.com>\r\nSubject: New release\r\nContent-Type: text/plain; charset=utf-8\r\n\r\nPrivate update";
  await worker.email({
    from: "outsider@example.com", to: "djparislife@ghostsinshells.com", raw,
    setReject: () => {}
  }, env);
  assert.equal(inserted, 0);
  await worker.email({
    from: "d.wright@ghostsinshells.com", to: "djparislife@ghostsinshells.com", raw,
    headers: new Headers({ "Message-ID": "<one@example.com>" }),
    setReject: () => {}
  }, env);
